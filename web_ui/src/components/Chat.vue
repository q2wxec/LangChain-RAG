          ...
          <li v-for="(item, index) in QA_List" :key="index">
            ...
            <div v-else class="ai">
              <div class="content">
                ...
                <div v-if="item.kb_name" :class="['avatar', 'text-avatar']">
                  <span>{{ item.kb_name.slice(0, 2) }}</span>
                  <br />
                  <span v-if="item.kb_name.length > 2">{{ item.kb_name.slice(2, 4) }}</span>
                </div>
                <img v-else class="avatar" src="../assets/home/ai-avatar.png" alt="头像" />
                <p
                  class="question-text"
                  :class="[
...
const like = useThrottleFn((item, e) => {
  item.like = !item.like;
  item.unlike = false;
  _czc.push(['_trackEvent', 'LangChain-RAG', '问答页面', '点赞', '', '']);
  if (item.like) {
...
const unlike = (item: IChatItem) => {
  ...
  item.like = false;
  _czc.push(['_trackEvent', 'LangChain-RAG', '问答页面', '点踩', '', '']);
};
...
const addAnswer = (question: string, kb_name: string) => {
  QA_List.value.push({
    answer: '',
    question,
    type: 'ai',
...
const getKbName = (kb_id: string) => {
  //遍历knowledgeBaseList获取KbName
  for (let i = 0; i < knowledgeBaseList.value.length; i++) {
    if (knowledgeBaseList.value[i].kb_id === kb_id) {
      return knowledgeBaseList.value[i].kb_name;
    }
  }
};
...
const send = () => {
  ...
  if (showLoading.value) {
    return message.warning('上轮回复还未完毕，请耐性等待回复完毕后，再次提问！');
  }
  ...
  async function processSingleKB(kb_ids, kb_id) {
  ...
  // 创建一个函数用来按顺序处理所有kb_id
  async function processKBsSequentially(kb_ids) {
    for (const kb_id of kb_ids) {
      await processSingleKB(kb_ids, kb_id);
      await new Promise(resolve => setTimeout(resolve, 200));
    }
    console.log('所有kb_id的请求已严格按顺序执行完毕');
    showLoading.value = false;
  }
  let kb_ids = selectList.value;
  // 使用
  processKBsSequentially(kb_ids).catch(err => {
    console.error('在处理过程中发生错误:', err);
    showLoading.value = false;
  });
};
